...
def update_spawn(self):
	self.tick -= 1
	if self.tick < 0:
		_, x, y, _ = self.object_entry[4:]
		if x > Globs.musashi.x:
			self.x = camera.virtual_right
		else:
			self.x = camera.virtual_left

		if collides_background(self, self.front, 1) != 0\
		and collides_background(self, self.back, 1) != 0\
		and collides_background(self, self.front, 0) == 0\
		and collides_background(self, self.back, 0) == 0:
...
def release(self):
	...
	print ('punk release: %s' % self.object_entry)
